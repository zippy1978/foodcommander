            ...
            process.nextTick(function() {
                ...
                if (response.statusCode === 200) {
                    ...
                    if ($('.form-cart-confirm').length > 0) {
                        ...
                        // TODO (BUG FIX) : get formatData.hours and formData.minutes 
                        // from first value of select fields 'hours' and 'minutes' of page /fr/cart/confirm
                        
                        confirmation.estimatedDeliveryDate = moment(sprintf('%s %s',moment().format('YYYY-MM-DD'), storeInfo.actualDeliveryTime));
...
var confirmOrder = function(orderConfirmation, cookies) {
    ...
    // Seems posted date must be rounded to next ten minutes from estimated date
    // TODO = rename estimatedDeliveryDate because it is not a date !
    var roundedDeliveryDate = timeUtil.roundToNextTenMinutes(new Date(orderConfirmation.estimatedDeliveryDate));
    ...
    var requestOptions = {
        ...
        formData: {
            payment: 'card',
            when: 'asap',
            hours: roundedDeliveryDate.getHours().toString(),
            minutes: ('0' + roundedDeliveryDate.getMinutes()).slice(-2),
